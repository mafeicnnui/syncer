...
def get_sync_where_incr(tab):
    ...
    v='where {0} >=DATEADD(DAY,-{1},GETDATE())'.format(v_rq,v_expire_day)
    if tab.split(':')[1]=='':
       return ''
    else:
       return v
...
def sync_sqlserver_data(config,debug):
    ...
    for ftab in config['sync_table'].split(","):
        if not check_full_sync(config, ftab.split(':')[0]):
            ...
            db_desc          = config['db_mysql']
            cr_desc          = db_desc.cursor()
            v_sql            = """select {0} from {1} with(nolock) {2} order by {3}""".format(v_pk_cols, tab,v_where,v_pk_names)
            n_rows           = 0
            cr_source.execute(v_sql)
            rs_source        = cr_source.fetchmany(n_batch_size)
            start_time       = datetime.datetime.now()
            ...
            if ftab.split(':')[1]=='':
                ...
                write_log(config,"Sync Table increment :{0} for In recent {1} days...".format(ftab.split(':')[0], ftab.split(':')[2]))
            ...
            while rs_source:
                ...
                if (sqlserver_rows_string_batch != mysql_rows_string_batch) :
                    for i in range(len(rs_source)):
                        ...
                        #print("sqlserver_rows_string is ok")
                        mysql_rows_string     = get_mysql_row_strings(config,tab,pkid)
                        #print("mysql_rows_string is ok")
            ...
            db_source2.commit()
            if i_counter!=0 :
                print("Time:{0},Table:{1},Total rec:{2},Process rec:{3}(tab:{4},update:{5}),elapsed time:{6}s". \
                       format(get_time(), tab,n_tab_total_rows, i_counter,
                              get_mapping_tname(tab), i_counter_upd,str(get_seconds(start_time)), end=''))

                write_log(config,"Time:{0},Table:{1},Total rec:{2},Process rec:{3}(tab:{4},update:{5}),elapsed time:{6}s". \
                       format(get_time(), tab, n_tab_total_rows, i_counter,
                              get_mapping_tname(tab), i_counter_upd, str(get_seconds(start_time))),False)

            #sync delete
            write_log(config,'Comparing Table :{0} SQLServer and MySQL primary key please wait...'.format(tab),True)
            sync_incr_delete(config,ftab)

def sync_sqlserver_full_data(config):
    db_desc         = config['db_mysql']
    cr_desc         = db_desc.cursor()
    for ftab in config['sync_table'].split(","):
        tab = ftab.split(':')[0]
        if check_full_sync(config, tab):
           sql='truncate table {0}'.format(get_mapping_tname(tab))
           cr_desc.execute(sql)
           print(sql+'...ok')
           sync_sqlserver_init_full_tab(config,get_mapping_tname(tab))
    #sync_sqlserver_init(config,debug)
    print('{0}...full sync complete'.format(get_mapping_tname(tab)))
    db_desc.commit()
    cr_desc.close()
    ...
    while True:
      ...
      if config['full_sync_method'] == 'timing':
          if get_now() >= config['full_sync_time'] and not check_full_sync_finish(config):
              sync_sqlserver_full_data(config)
      ...
      write_log(config, "set sync status is ready[1]!")

      #send mail
      #if get_seconds(start_mail_time) >= int(config['mail_gap']):
      send_mail25(config['send_user'], config['send_pass'], config['acpt_user'], config['mail_title'], get_html_contents(config))
      start_mail_time = datetime.datetime.now()
