...
# @Software: PyCharm
# 功能：通过删除最近天数的数据，再次上传的方式进行同步(实时车流版）
import sys,time
import traceback
import configparser
import warnings
...
def sync_sqlserver_ddl(config,debug):
    ...
    #创建MySQL会话临时表
    '''
    cr_desc.execute("""set autocommit=1""")
    cr_desc.execute("""
       create temporary table sync.t_sync_tmp
        (v0 VARCHAR(100),v1 VARCHAR(100),v2 VARCHAR(100),
         v3 VARCHAR(100),v4 VARCHAR(100),v5 VARCHAR(100),
         v6 VARCHAR(100),v7 VARCHAR(100),v8 VARCHAR(100))
     """)
    write_log(config, "Temporary Table:{0} creating success!".format('sync.t_sync_tmp'))
    db_desc.commit()
    '''
    cr_source.close()
    cr_desc.close()
...
def sync_sqlserver_data_pk(config,ftab,config_init):
    ...
    if not check_full_sync(config, ftab.split(':')[0]) and not config_init[ftab.split(':')[0]]:
        ...
        rs_source        = cr_source.fetchmany(n_batch_size)
        start_time       = datetime.datetime.now()
        #print('sync_sqlserver_data_pk=',v_sql)
        if ftab.split(':')[1]=='':
        ...
        if ftab.split(':')[1] == '':
            write_log(config,'DB:{0},delete {1} table data please wait...'.format(config['db_mysql_string'], get_mapping_tname(tab)),True)
            cr_desc.execute('delete from {0}'.format(get_mapping_tname(tab)))
            write_log(config,'DB:{0},delete {1} table data ok!'.format(config['db_mysql_string'], get_mapping_tname(tab)),True)
        ...
        while rs_source:
            ...
            if len(rs_source) > 0:
                ...
                if n_tab_total_rows == 0:
                    write_log(config, "Table:{0},Total rec:{1},Process rec:{2},Complete:{3}%,elapsed time:{4}s"
                              .format(tab, n_tab_total_rows, i_counter, round(i_counter / 1 * 100, 2),str(get_seconds(start_time))), False)
                else:
                    write_log(config, "Table:{0},Total rec:{1},Process rec:{2},Complete:{3}%,elapsed time:{4}s"
                              .format(tab, n_tab_total_rows, i_counter, round(i_counter / n_tab_total_rows * 100, 2),str(get_seconds(start_time))), False)
                rs_source = cr_source.fetchmany(n_batch_size)
            print('')
        db_desc.commit()
...
def sync_sqlserver_data_nopk(config,ftab,config_init):
    ...
    if  check_full_sync(config, ftab.split(':')[0]) and not config_init[ftab.split(':')[0]]:
        ...
        v_sql            = """select {0} from {1} with(nolock) {2}
                           """.format(get_sync_table_cols(config,tab), tab,v_where)
        n_rows           = 0
        cr_source.execute(v_sql)
        rs_source        = cr_source.fetchmany(n_batch_size)
        start_time       = datetime.datetime.now()
        #print('sync_sqlserver_data_nopk=',v_sql)
        if ftab.split(':')[1]=='':
            write_log(config,"Sync Table increment :{0} ...".format(ftab.split(':')[0]),True)
        else:
            write_log(config,"Sync Table increment :{0} for In recent {1} {2}...".format(ftab.split(':')[0], ftab.split(':')[2],config['sync_time_type']),True)
        ...
        if ftab.split(':')[1] == '':
            ...
            write_log(config,'DB:{0},delete {1} table recent {2} {3} data please wait...'.format(config['db_mysql_string'], tab,ftab.split(':')[2],config['sync_time_type']),True)
            cr_desc.execute('delete from {0} {1} '.format(get_mapping_tname(tab), v_where_mysql))
            write_log(config,'DB:{0},delete {1} table recent {2} {3} data ok!'.format(config['db_mysql_string'], tab,ftab.split(':')[2],config['sync_time_type']),True)
        ...
        while rs_source:
            ...
            print("\r{0},Scanning table:{1},{2}/{3} rows,elapsed time:{4}s..."
                  .format(get_time(),get_mapping_tname(tab),str(n_rows),str(n_tab_total_rows),str(get_seconds(start_time))),end='')
            rs_source_desc = cr_source.description
            if len(rs_source) > 0:
                for r in list(rs_source):
                    ins_val = ""
                ...
                print("\rTable:{0},Total rec:{1},Process rec:{2},Complete:{3}%,elapsed time:{4}s"
                      .format(tab, n_tab_total_rows, i_counter, round(i_counter / n_tab_total_rows * 100, 2),str(get_seconds(start_time))),end='')
                if n_tab_total_rows == 0:
                    write_log(config, "Table:{0},Total rec:{1},Process rec:{2},Complete:{3}%,elapsed time:{4}s"
                              .format(tab, n_tab_total_rows, i_counter, round(i_counter / 1 * 100, 2),str(get_seconds(start_time))), False)
                else:
                    write_log(config, "Table:{0},Total rec:{1},Process rec:{2},Complete:{3}%,elapsed time:{4}s"
                              .format(tab, n_tab_total_rows, i_counter, round(i_counter / n_tab_total_rows * 100, 2),str(get_seconds(start_time))),
                              False)
                rs_source = cr_source.fetchmany(n_batch_size)
            db_desc.commit()
            print('')
