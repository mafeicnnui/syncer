...
def sync_after_prcess(config,ftab):
    ...
    db_dest.commit()
    cr_dest.close()
    write_log(config,"DB:{0},Table :{1} sync after process complete!".format(config['db_mysql_string'],get_mapping_tname(tab)))
...
def sync_sqlserver_data(config,debug):
    ...
    for ftab in config['sync_table'].split(","):
        if not check_full_sync(config, ftab.split(':')[0]):
            ...
            #sync delete
            write_log(config,'Comparing Table :{0} SQLServer and MySQL primary key please wait...'.format(tab),False)
